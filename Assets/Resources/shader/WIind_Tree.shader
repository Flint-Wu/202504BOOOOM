...
Shader "Polygon Wind/Tree" {
    ...
    Properties {
        ...
        _wind_dir ("风向", Vector) = (0.5,0.05,0.5,0)
        _wind_size ("风浪大小", range(5,50)) = 5

        _tree_sway_stutter_influence("树摇晃停顿影响度", range(0,1)) = 0.2
        _tree_sway_stutter ("树摇晃停顿", range(0,10)) = 1.5
        _tree_sway_speed ("树摇晃速度", range(0,10)) = 1
        _tree_sway_disp ("树摇晃位移", range(0,1)) = 0.3

        _branches_disp ("树枝位移", range(0,0.5)) = 0.3

        _leaves_wiggle_disp ("树叶摆动位移", float) = 0.02
        _leaves_wiggle_speed ("树叶摆动速度", float) = 0.002
