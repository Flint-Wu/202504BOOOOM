Shader "Hidden/Locked/.poiyomi/Old Versions/8.1/Poiyomi Toon/bd64ff25d55580d4babac484ae4eb86a"
{
 ...
 #define OPTIMIZER_ENABLED 
			...
			void ApplyFPSText(inout PoiFragData poiFragData, float2 uv, in PoiMods poiMods)
			{
				...
				sigDist *= max(dot(msdfUnit, 0.5 / fwidth(uv)), 1);
				float opacity = clamp(sigDist + 0.5, 0, 1);
				poiFragData.baseColor = lerp(poiFragData.baseColor, poiThemeColor(poiMods, float4(0,1,0.9905875,1).rgb, (0.0 /*_TextFPSColorThemeIndex*/)), opacity * float4(0,1,0.9905875,1).a);
				globalTextEmission += poiThemeColor(poiMods, float4(0,1,0.9905875,1).rgb, (0.0 /*_TextFPSColorThemeIndex*/)) * opacity * (2.0 /*_TextFPSEmissionStrength*/);
			}
